...
export function ArticleManager({ initialCategoryId }: ArticleManagerProps) {
  ...
  const filteredArticles = articles.filter((article) => {
    ...
    if (activeCategoryId && activeCategoryId !== 'all' && article.category_id !== activeCategoryId) {
      return false;
    }
    if (filterStatus && filterStatus !== 'all') {
      if (filterStatus === 'draft' && article.is_draft) return false;
      if (filterStatus === 'published' && !article.is_draft) return false;
    }
    return true;
  });
        ...
        {filteredArticles.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-10 text-center border rounded-md bg-muted/10">
            ...
            <p className="text-muted-foreground mb-4">
              {searchQuery || activeCategoryId !== 'all' || filterStatus !== 'all'
                    ...
                    <CardFooter className="flex justify-between items-center text-sm text-muted-foreground pt-0">
                      <div className="flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full ${
                          article.is_draft ? 'bg-amber-500' : 'bg-green-500'
                        }`} />
                        <span className="capitalize">{article.is_draft ? 'draft' : 'published'}</span>
                      </div>
                      <div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Select
                        value={currentArticle.is_draft ? 'draft' : 'published'}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex gap-2">
                        ...
                        {/* CloudinaryUploader and CloudinaryMediaLibrary components need to be implemented */}
                        <Button
                          variant="outline"
                          onClick={() => {
                            toast({
                              title: "Media upload coming soon",
                              description: "Media upload functionality will be implemented soon",
                            });
                          }}
                        >
                        ...
                        <Button
                          ...
                          onClick={() => {
                            toast({
                              ...
                              description: "Media library functionality will be implemented soon",
                            });
                          }}
                        >
