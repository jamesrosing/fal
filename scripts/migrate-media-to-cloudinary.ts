...
async function migrateMediaToCloudinary() {
  ...
  const staticMigrations: MediaAssetRecord[] = [];
  
  for (const [id, asset] of Object.entries(IMAGE_ASSETS) as [string, MediaAsset][]) {
  ...
  if (!tableCheckError) {
    ...
    if (fetchError) {
      ...
      const placeholderMigrations: MediaAssetRecord[] = (existingAssets || []).map((asset: PlaceholderAsset) => ({
  ...
  Object.entries(IMAGE_ASSETS).forEach(([id, asset]: [string, MediaAsset]) => {
  ...
  if (!tableCheckError) {
    ...
    if (placeholders && placeholders.length > 0) {
      placeholders.forEach((placeholder: PlaceholderAsset) => {
