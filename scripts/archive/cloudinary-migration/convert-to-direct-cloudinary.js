/**
 * This script migrates from placeholder-based media components to direct Cloudinary components
 * It searches for UnifiedMedia, Media, and other placeholder-based components and replaces them
 * with direct CldImage or CldVideo components.
 */
...
async function processImports(filePath, content) {
  ...
  // Replace media imports with Cloudinary imports
  ...
  if (hasMediaImports) {
    ...
    // Check if we need to add Cloudinary imports
    const hasUnifiedMedia = /<UnifiedMedia/g.test(content);
    
    // Add next-cloudinary import if needed
    if (hasUnifiedMedia) {
      newContent = newContent.replace(
        IMPORT_REGEX,
        (match, imports, quote1, quote2) => {
          // Check if CldImage or CldVideo is needed
          const needsCldImage = !content.includes('CldImage') && content.includes('UnifiedMedia');
          const addImports = [];
          ...
          const importStatement = `import { ${addImports.join(', ')} } from 'next-cloudinary';\nimport { ${imports.trim()} } from ${quote1}@/components/media${quote2}`;
